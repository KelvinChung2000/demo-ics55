...
ECC takes its settings at two layers and only the first is a file the project
owns. `ecc.toml`'s `[params.<section>]` tables reach ECC's parameter registry,
which is the thirteen entries `ecc param list` prints and nothing else;
everything outside that registry, the die size and the PDN stripe pitch above
all, exists only in the workspace's own step configs and is patched there after
synthesis by `flow.project.patch_floorplan`. This module covers the first layer
and names the handful of values the second layer is patched from.
...
A die is the one setting that does not reach ECC through `ecc.toml`. A
`Tile/<type>/ecc_config.toml` may carry a `[die]` table, and `flow.plan` binds
it to the columns and rows that type occupies before deriving every pin offset
and seam from the result, so `flow.verify` still proves the fabric meets on the
die that was asked for. Writing it into `ecc.toml` instead would put it past the
only check that can tell whether it abuts, which is why `gds_config.yaml`'s
per-tile `DIE_AREA` has no equivalent here.
"""
...
# `ecc.toml`, where ECC would silently ignore it.
